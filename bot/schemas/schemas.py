...
class Open(BaseModel):
    ...
    srank_cost: int = 5
    # 注册人数分级限制（0=不限制）
    all_user_b: int = 0   # 普通用户人数上限
    all_user_e: int = 0   # 公益用户人数上限
    all_user_a: int = 0   # 白名单人数上限
    # 求片额度设置（0=禁止求片，>0=每周期允许求片数量）
    request_quota_a: int = 999
...
class CmdOverride(BaseModel):
    """单条命令的覆盖配置"""
    visible: bool = True          # 是否显示在命令列表中
    min_level: str = "user"       # 最低可见级别: "user" / "admin" / "owner"
...
class Config(BaseModel):
    ...
    game: Optional[Dict] = Field(default_factory=dict)

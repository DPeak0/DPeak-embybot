...
"""
import asyncio
from bot import owner, admins, group, LOGGER, bot
...
class BotCommands:
    ...
    async def set_commands(client):
        ...
        from bot.web.admin.api_commands import build_command_lists
        try:
    ...
    async def pro_commands(client, uid):
        ...
        try:
            overrides = config.cmd_overrides or {}
            _, admin_p, _ = build_command_lists(overrides)
            try:
    ...
    async def rev_commands(client, uid):
        ...
        try:
            overrides = config.cmd_overrides or {}
            user_p, _, _ = build_command_lists(overrides)
            try:

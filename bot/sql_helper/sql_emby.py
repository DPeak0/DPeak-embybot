...
class Emby(Base):
    ...
    lv: a=白名单, b=普通用户, c=已封禁, d=未注册, e=公益用户
    base_lv: 记录用户的基础等级（封禁前），用于解封时恢复
    """
    __tablename__ = 'emby'
    ...
    base_lv = Column(String(1), nullable=True)  # 基础等级，封禁时保留原始等级
...
def _migrate_add_base_lv():
    """迁移：若 base_lv 列不存在则自动添加"""
    try:
        with engine.connect() as conn:
            result = conn.execute(text(
                "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS "
                "WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='emby' AND COLUMN_NAME='base_lv'"
            ))
            if result.scalar() == 0:
                conn.execute(text(
                    "ALTER TABLE emby ADD COLUMN base_lv VARCHAR(1) NULL DEFAULT NULL AFTER lv"
                ))
                conn.commit()
                LOGGER.info("【DB迁移】已添加 emby.base_lv 列")
    except Exception as e:
        LOGGER.error(f"【DB迁移】添加 base_lv 列失败: {e}")


_migrate_add_base_lv()
...
def sql_count_emby_by_lv() -> dict:
    """
    统计各等级人数
    :return: dict with keys: a, b, c, d, e (counts per lv)
    """
    with Session() as session:
        try:
            result = session.query(
                func.count(case((Emby.lv == "a", 1))).label("cnt_a"),
                func.count(case((Emby.lv == "b", 1))).label("cnt_b"),
                func.count(case((Emby.lv == "c", 1))).label("cnt_c"),
                func.count(case((Emby.lv == "d", 1))).label("cnt_d"),
                func.count(case((Emby.lv == "e", 1))).label("cnt_e"),
            ).first()
            return {"a": result.cnt_a, "b": result.cnt_b, "c": result.cnt_c,
                    "d": result.cnt_d, "e": result.cnt_e}
        except Exception as e:
            LOGGER.error(f"【DB】sql_count_emby_by_lv 失败: {e}")

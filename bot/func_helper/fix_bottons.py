...
from bot import chanel, main_group, bot_name, extra_emby_libs, tz_id, tz_ad, tz_api, tz_version, tz_username, tz_password, _open, sakura_b, \
    schedall, auto_update, fuxx_pitao, moviepilot, red_envelope, config, miniapp_url, LOGGER
...
def judge_start_ikb(is_admin: bool, account: bool) -> InlineKeyboardMarkup:
    ...
    if _open.checkin:
        # 配置了 miniapp_url 时，签到按钮改为 WebApp 形式（替换回调按钮）
        if not miniapp_url:
            d.append(['🎯 签到', 'checkin'])
...
def store_ikb():
    return ikb([[(f'🎟️ 兑换注册码', 'store-invite'), (f'🔍 查询注册码', 'store-query')],
                [(f'❌ 取消', 'members')]])
...
def gm_ikb_content(is_owner: bool = False) -> InlineKeyboardMarkup:
    rows = []
    if is_owner:
        rows.append([('⭕ 注册状态', 'open-menu'), ('🎟️ 注册/续期码', 'cr_link')])
    else:
        rows.append([('🎟️ 注册/续期码', 'cr_link')])
    rows += [
        [('💊 查询注册', 'ch_link'), ('🏬 兑换设置', 'set_renew')],
        [('👥 普通用户', 'normaluser'), ('🟢 公益用户', 'publicuser'), ('👑 白名单', 'whitelist')],
        [('💠 设备列表', 'user_devices')],
    ]
    if is_owner:
        rows.append([('🌏 定时', 'schedall'), ('🕹️ 主界面', 'back_start'), ('其他 🪟', 'back_config')])
    else:
        rows.append([('🕹️ 主界面', 'back_start'), ('其他 🪟', 'back_config')])
    return ikb(rows)
...
def all_user_limit_ikb() -> InlineKeyboardMarkup:
    """注册人数分级限制选择键盘"""
    return ikb([
        [('📊 总人数限制', 'set_all_user_total')],
        [('🔵 普通用户上限', 'set_all_user_b'), ('🟢 公益用户上限', 'set_all_user_e')],
        [('♾ 白名单上限', 'set_all_user_a')],
        [('🔙 返回', 'open-menu')],
    ])
...
async def publicuser_page_ikb(total_page: int, current_page: int) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboard()
    keyboard.paginate(total_page, current_page, 'publicuser:{number}')
    next = InlineButton('⏭️ 后退+5', f'publicuser:{current_page + 5}')
    previous = InlineButton('⏮️ 前进-5', f'publicuser:{current_page - 5}')
    followUp = [InlineButton('🔙 Back', 'manage')]
    if total_page > 5:
        if current_page - 5 >= 1:
            followUp.append(previous)
...
def invite_lv_ikb():
    keyboard = ikb([
        ...
        [('🟢 公益用户', 'set_invite_lv-e'), ('©️ 已禁用用户', 'set_invite_lv-c')],
        [('🅳️  所有用户', 'set_invite_lv-d')],
        [('🔙 返回', 'set_renew')]
    ])
    return keyboard
...
def checkin_lv_ikb():
    keyboard = ikb([
        ...
        [('🟢 公益用户', 'set_checkin_lv-e'), ('©️ 已禁用用户', 'set_checkin_lv-c')],
        [('🅳️  所有用户', 'set_checkin_lv-d')],
        [('🔙 返回', 'set_renew')]
    ])
    return keyboard


def cr_link_type_ikb() -> InlineKeyboardMarkup:
    """注册/续期码类型选择按钮。所有管理员均可生成全部类型。"""
    return ikb([
        [('🟢 公益注册码', 'cr_link_type-e'), ('🔵 普通注册码', 'cr_link_type-b')],
        [('🎟️ 续期码', 'cr_link_type-renew')],
        [('🔙 返回', 'manage')]
    ])
""" config_panel ↓"""


def config_preparation(is_owner: bool = True) -> InlineKeyboardMarkup:

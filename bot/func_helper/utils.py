...
# 等级排序（数字越小权限越高）：a=白名单, b=普通用户, e=公益用户, c=已封禁, d=未注册
_LV_RANK = {'a': 0, 'b': 1, 'e': 1, 'c': 2, 'd': 3}


def lv_allowed(user_lv: str, min_lv: str) -> bool:
    """
    判断用户等级是否满足最低等级要求
    :param user_lv: 用户当前等级
    :param min_lv: 最低等级要求
    :return: True=满足, False=不满足
    """
    return _LV_RANK.get(user_lv, 99) <= _LV_RANK.get(min_lv, 0)
...
async def members_info(tg=None, name=None):
    ...
    if data is None:
        ...
        embyid = data.embyid
        iv = data.iv
        lv_dict = {'a': '白名单', 'b': '**正常**', 'c': '**已禁用**', 'd': '未注册', 'e': '**公益用户**'}
        ...
        if lv == '白名单':
            ...
            ex = data.ex or '无账户信息'
        return name, lv, ex, iv, embyid, pwd2, data.lv
...
# 创建注册
async def cr_link_one(tg: int, times, count, days: int, method: str, lv: str = 'e'):
    """
    创建注册码连接
    :return: (links_str, code_list) 或 (None, None)（数据库插入失败时）
    """
    links = ''
    code_list = []
    i = 1
    if method == 'code':
        while i <= count:
            ...
            uid = f'{ranks.logo}-{lv}-{times}-Register_{p}'
        ...
        while i <= count:
            ...
            links += link
            i += 1
    if sql_add_code(code_list, tg, days, lv=lv) is False:
        return None, None
    return links, code_list

...
from bot import bot, prefixes, bot_photo, Now, LOGGER, config, save_config, _open, auto_update, moviepilot, sakura_b, owner
...
async def config_p_set(_, msg):
    ...
    is_owner = msg.from_user.id == owner
    await sendPhoto(msg, photo=bot_photo, caption="🌸 欢迎回来！\n\n👇点击你要修改的内容。",
                    buttons=config_preparation(is_owner))
...
async def config_p_re(_, call):
    ...
    await editMessage(call, "🌸 欢迎回来！\n\n👇点击你要修改的内容。", buttons=config_preparation(is_owner))
...
async def set_tz_version_v0(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              ...
              before_val=old_ver, after_val='v0', detail="探针版本切换")
    await set_tz(_, call)
...
async def set_tz_version_v1(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=old_ver, after_val='v1', detail="探针版本切换")
    await set_tz(_, call)
...
async def set_tz_version_komari(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=old_ver, after_val='komari', detail="探针版本切换")
    await set_tz(_, call)
    ...
    if txt is False:
        ...
        try:
            ...
            if config.tz_version == 'v0':
                ...
                log_audit(category="settings", action="update", source="bot",
                          operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                          detail=f"更新探针参数(komari)：地址={s_tz}")
...
async def set_emby_line(_, call):
    ...
    if txt is False:
        ...
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  after_val=config.emby_line, detail="Emby线路变更")
...
async def set_whitelist_emby_line(_, call):
    ...
    if txt is False:
        ...
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  after_val=config.emby_whitelist_line, detail="白名单Emby线路变更")
...
async def set_block(_, call):
    ...
    if txt is False:
        ...
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  after_val=str(config.emby_block), detail="媒体库显隐设置变更")
...
async def set_auto_update(_, call):
    try:
        ...
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  before_val=str(not auto_update.status), after_val=str(auto_update.status),
                  detail="自动更新开关切换")
...
async def set_mp_status(_, call):
    ...
    try:
        ...
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  before_val=str(not moviepilot.status), after_val=str(moviepilot.status),
                  detail="MoviePilot点播功能开关切换")
...
async def set_mp_price(_, call):
    ...
    try:
        ...
        if price < 0:
            raise ValueError
        old_price = moviepilot.price
        moviepilot.price = price
        save_config()
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  before_val=str(old_price), after_val=str(price),
                  detail="MoviePilot点播价格变更")
...
async def set_mp_lv(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val='b' if moviepilot.lv == 'a' else 'a', after_val=moviepilot.lv,
              detail="MoviePilot用户权限切换")
    await mp_config_panel(_, call)
...
async def set_mp_log_channel(_, call):
    ...
    try:
        ...
        old_channel = moviepilot.download_log_chatid
        moviepilot.download_log_chatid = chat_id
        save_config()
        log_audit(category="settings", action="update", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  before_val=str(old_channel), after_val=str(chat_id),
                  detail="MoviePilot日志频道变更")
...
async def open_leave_ban(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=str(not _open.leave_ban), after_val=str(_open.leave_ban),
              detail="退群封禁开关切换")
...
async def set_user_playrank(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=str(not _open.uplays), after_val=str(_open.uplays),
              detail="观影榜结算开关切换")
...
async def set_kk_gift_days(_, call):
    ...
    if txt is False:
        ...
        try:
            ...
            log_audit(category="settings", action="update", source="bot",
                      operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                      after_val=str(days), detail="kk赠送资格天数变更")
...
async def set_fuxx_pitao(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=str(not config.fuxx_pitao), after_val=str(config.fuxx_pitao),
              detail="皮套过滤开关切换")
...
async def set_red_envelope_status(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=str(not config.red_envelope.status), after_val=str(config.red_envelope.status),
              detail="红包功能开关切换")
...
async def set_red_envelope_allow_private(_, call):
    ...
    log_audit(category="settings", action="update", source="bot",
              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
              before_val=str(not config.red_envelope.allow_private), after_val=str(config.red_envelope.allow_private),
              detail="专属红包开关切换")
...
async def set_activity_check_days(_, call):
    ...
    if txt is False:
        ...
        try:
            ...
            log_audit(category="settings", action="update", source="bot",
                      operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                      after_val=str(days), detail="活跃检测天数变更")

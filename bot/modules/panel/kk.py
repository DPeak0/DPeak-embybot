        ...
        if e.lv != "c":
            if await emby.emby_change_policy(emby_id=e.embyid, disable=True) is True:
                if sql_update_emby(Emby.tg == b, lv='c') is True:
                    ...
                    LOGGER.info(text)
                    log_audit(category="account", action="ban", source="bot",
                              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                              target_tg=b, target_name=first.first_name,
                              before_val=e.lv, after_val="c",
                              detail=f"Bot管理员封禁用户 {e.name}")
                else:
            ...
            if await emby.emby_change_policy(emby_id=e.embyid):
                if sql_update_emby(Emby.tg == b, lv='b'):
                    text += '解禁完成'
                    LOGGER.info(text)
                    log_audit(category="account", action="unban", source="bot",
                              operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                              target_tg=b, target_name=first.first_name,
                              before_val="c", after_val="b",
                              detail=f"Bot管理员解禁用户 {e.name}")
                else:
    ...
    if success:
        try:
            ...
            if re is True:
                ...
                log_audit(category="account", action="extralib_unblock", source="bot",
                          operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                          target_tg=tgid, target_name=e.name,
                          detail=f"Bot管理员开启额外媒体库 {extra_emby_libs}")
            else:
    ...
    if success:
        try:
            ...
            if re is True:
                ...
                log_audit(category="account", action="extralib_block", source="bot",
                          operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                          target_tg=tgid, target_name=e.name,
                          detail=f"Bot管理员关闭额外媒体库 {extra_emby_libs}")
            else:
                await editMessage(call,
                                  f'🌧️ Error！管理员 [{call.from_user.first_name}](tg://user?id={call.from_user.id})\n操作失败请检查设置！')
        except Exception as e:
...
async def gift(_, call):
    ...
    if e.embyid is None:
        ...
        log_audit(category="code", action="gift", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  target_tg=b, target_name=first.first_name,
                  detail=f"Bot管理员赠送注册资格，有效天数={config.kk_gift_days}")
    else:
...
async def close_emby(_, call):
    ...
    if await emby.emby_del(emby_id=e.embyid):
        ...
        log_audit(category="account", action="delete", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  target_tg=b, target_name=first.first_name,
                  before_val=e.lv, after_val="d",
                  detail=f"Bot管理员删除账户 {e.name}（embyid={e.embyid}）")
    else:
...
async def fuck_off_m(_, call):
    ...
    try:
        ...
        log_audit(category="account", action="kick", source="bot",
                  operator_tg=call.from_user.id, operator_name=call.from_user.first_name,
                  target_tg=user_id, target_name=user.first_name,
                  detail=f"Bot管理员踢出群组 {call.message.chat.id}")

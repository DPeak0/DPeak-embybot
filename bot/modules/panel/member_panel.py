...
from bot.sql_helper.sql_emby import sql_get_emby, sql_update_emby, Emby, sql_delete_emby, sql_count_emby_by_lv
...
async def create_user(_, call, us, stats):
    ...
    try:
        ...
        async with _create_user_lock:
            ...
            # 分级人数检查（0=不限制）
            e_row = sql_get_emby(tg=msg.from_user.id)
            pending_lv = (e_row.lv if e_row and e_row.lv not in ('c', 'd') else 'b') if e_row else 'b'
            ...
            if pending_lv == 'b' and lim_b > 0 and lv_counts.get('b', 0) >= lim_b:
                return await msg.reply(f'**🚫 普通用户名额已满（{lv_counts.get("b",0)}/{lim_b}），请联系管理员。**')
            elif pending_lv == 'e' and lim_e > 0 and lv_counts.get('e', 0) >= lim_e:
                return await msg.reply(f'**🚫 公益用户名额已满（{lv_counts.get("e",0)}/{lim_e}），请稍后再试。**')
            elif pending_lv == 'a' and lim_a > 0 and lv_counts.get('a', 0) >= lim_a:
                return await msg.reply(f'**🚫 白名单名额已满（{lv_counts.get("a",0)}/{lim_a}），请联系管理员。**')
            ...
            if not data:
                ...
                # 数据库操作：保留用户已设置的等级（公益用户/普通用户），open注册统一为'b'
                if stats:
                    target_lv = 'b'
                else:
                    # 码注册：使用exchange.py已写入的lv（e或b），若仍为'd'则默认'b'
                    current_lv = sql_get_emby(tg=tg)
                    target_lv = current_lv.lv if current_lv and current_lv.lv in ('b', 'e') else 'b'
                if stats:
                    sql_update_emby(Emby.tg == tg, embyid=eid, name=emby_name, pwd=pwd, pwd2=emby_pwd2, lv=target_lv, base_lv=target_lv, cr=datetime.now(), ex=ex)
                else:
                    sql_update_emby(Emby.tg == tg, embyid=eid, name=emby_name, pwd=pwd, pwd2=emby_pwd2, lv=target_lv, base_lv=target_lv, cr=datetime.now(), ex=ex, us=0)
                ...
                log_audit(category="account", action="register", source="bot",
                          target_tg=call.from_user.id,
                          target_name=call.from_user.first_name,
                          after_val=target_lv,
                          detail=f"创建账户 {emby_name}（embyid={eid}），等级={target_lv}，来源={'开注' if stats else '注册码'}")
...
async def del_emby(_, call):
    ...
    if await emby.emby_del(emby_id=embyid):
        ...
        log_audit(category="account", action="self_delete", source="bot",
                  target_tg=call.from_user.id, target_name=call.from_user.first_name,
                  detail=f"用户自主删除账户（embyid={embyid}）")
    else:
            ...
            if m.text != e.pwd2:
                ...
                if mima is False:
                    ...
                    if await emby.emby_reset(emby_id=e.embyid) is True:
                        ...
                        log_audit(category="account", action="reset_pwd", source="bot",
                                  target_tg=call.from_user.id, target_name=call.from_user.first_name,
                                  detail="用户自主重置密码为空")
                    else:
...
async def do_store_reborn(_, call):
    ...
    await callAnswer(call,
                     ...
                     True)
    if all([e.lv == 'c', e.base_lv == 'e', e.iv >= _open.exchange_cost, schedall.low_activity]):
        await editMessage(call,
        ...
        if m is False:
            ...
            sql_update_emby(Emby.tg == call.from_user.id, iv=e.iv - _open.exchange_cost, lv=e.base_lv or 'e')
            await emby.emby_change_policy(emby_id=e.embyid)
            LOGGER.info(f'【兑换解封】- {call.from_user.id} 已花费 {_open.exchange_cost}{sakura_b},解除封禁')
            log_audit(category="credits", action="exchange_unban", source="bot",
                      target_tg=call.from_user.id, target_name=call.from_user.first_name,
                      before_val=str(e.iv), after_val=str(e.iv - _open.exchange_cost),
                      detail=f"积分兑换解封，扣除={_open.exchange_cost}，恢复等级={e.base_lv or 'e'}")
            await asyncio.gather(m.delete(), do_store(_, call),
                                 sendMessage(call, '解封成功<(￣︶￣)↗[GO!]\n此消息将在20s后自焚', timer=20))
    else:
        await sendMessage(call, '❌ 不满足以下要求！ヘ(￣ω￣ヘ)\n\n'
                                '1. 被封禁的**公益用户**账户（普通用户不支持此功能）\n'
...
async def do_store_whitelist(_, call):
    if _open.whitelist:
        ...
        log_audit(category="credits", action="exchange_whitelist", source="bot",
                  target_tg=call.from_user.id, target_name=call.from_user.first_name,
                  before_val=f"{e.lv}/{e.iv}", after_val=f"a/{e.iv - _open.whitelist_cost}",
                  detail=f"积分兑换白名单，扣除={_open.whitelist_cost}")
    else:
...
async def do_store_invite(_, call):
    if _open.invite:
        ...
        # 允许 a/b/e 等活跃等级兑换（用 lv_allowed 替代字符串比较）
        ...
        await editMessage(call,
                          ...
                          f'**注意**：兑率 30天 = {_open.invite_cost}{sakura_b}，仅可生成**公益注册码**\n'
        ...
        try:
            ...
            # 固定生成公益注册码（lv='e'）
            links, code_list = await cr_link_one(call.from_user.id, days, count, days, method, lv='e')
            if links is None:
                return await editMessage(call, '⚠️ 数据库插入失败，请检查数据库')
            links = f"🎯 {bot_name}已为您生成了 **{days}天** 公益注册码 {count} 个\n\n" + links
            chunks = [links[i:i + 4096] for i in range(0, len(links), 4096)]
            for chunk in chunks:
                await sendMessage(content, chunk)
            LOGGER.info(f"【注册码兑换】：{bot_name}已为 {content.from_user.id} 兑换了 {count} 个 {days} 天公益注册码")
            log_audit(category="credits", action="exchange_code", source="bot",
                      target_tg=call.from_user.id, target_name=call.from_user.first_name,
                      before_val=str(e.iv), after_val=str(e.iv - cost),
                      detail=f"积分兑换公益注册码 {count} 个，{days}天，扣除={cost}\n码列表：\n" + '\n'.join(code_list))
    else:
        await callAnswer(call, '❌ 管理员未开启此兑换', True)

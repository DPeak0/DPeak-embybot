...
action_dict = {
    ...
    "check_miniapp": check_miniapp_requests,
}
...
args_dict = {
    ...
    "check_miniapp": {'minute': '*/30', 'id': 'check_miniapp_requests'},
}
...
async def sched_change_policy(_, call):
    try:
        ...
        old_val = getattr(schedall, method)
        if old_val:
            scheduler.remove_job(job_id=args['id'], jobstore='default')
        else:
            scheduler.add_job(action, 'cron', **args)
        new_val = not old_val
        setattr(schedall, method, new_val)
        save_config()

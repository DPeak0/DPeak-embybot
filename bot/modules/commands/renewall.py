...
async def renew_all(_, msg):
    ...
    if sql_update_embys(some_list=ls, method='ex'):
        ...
        log_audit(category="account", action="renew", source="bot",
                  operator_tg=msg.from_user.id, operator_name=msg.from_user.first_name,
                  detail=f"批量调整到期时间 {'+' if a >= 0 else ''}{a} 天，影响用户数={b}")
        for l in ls:
...
async def coins_all(_, msg):
    ...
    if sql_update_embys(some_list=ls, method='iv'):
        ...
        log_audit(category="credits", action="update", source="bot",
                  operator_tg=msg.from_user.id, operator_name=msg.from_user.first_name,
                  detail=f"批量调整积分 {'+' if coin >= 0 else ''}{coin}，等级={lv}，影响用户数={b}")
...
async def coinsclear(_, msg):
    ...
    if level_param == 'all':
        ...
        if rst:
            ...
            log_audit(category="credits", action="delete", source="bot",
                      operator_tg=msg.from_user.id, operator_name=msg.from_user.first_name,
                      detail="批量清除所有用户积分（coinsclear all）")
        else:
            await send.edit(f"⚡【{sakura_b}任务】\n\n  清除所有用户币币失败")
    ...
    # 根据等级清除币币
        ...
        if sql_update_embys(some_list=ls, method='iv'):
            ...
            log_audit(category="credits", action="delete", source="bot",
                      operator_tg=msg.from_user.id, operator_name=msg.from_user.first_name,
                      detail=f"批量清除积分（等级={lv}），影响用户数={count}")
        else:
            await send.edit(f"⚡【{sakura_b}任务】\n\n  清除{lv}级用户币币失败")
